...
from models.gv2 import Ghostblockv2
...
class TCSP_Ghost2(nn.Module):
    def __init__(self, input_channels, output_channels, num_blocks=1, shortcut=False, groups=1, expansion=0.5):
        super().__init__()
        self.c1_ = 16
        self.c2_ = 16 * expansion
        c_ = int(output_channels * expansion)
        hidden_channels = int(output_channels * expansion)
        self.conv1 = Conv(input_channels, 2 * hidden_channels, k=1, s=1)
        self.conv2 = Conv((2 + num_blocks) * hidden_channels, output_channels, k=1)
        self.blocks = nn.ModuleList(Ghostblockv2(c_, self.c1_, c_) for _ in range(num_blocks))

    def forward(self, x):
        features = self.conv1(x)
        features_split = torch.chunk(features, 2, dim=1)
        outputs = list(features_split)
        for block in self.blocks:
            last_output = outputs[-1]
            block_output = block(last_output)
            outputs.append(block_output)
        concatenated = torch.cat(outputs, dim=1)
        output = self.conv2(concatenated)
        return output
...
class MobileOne(nn.Module):
    # MobileOne
    def __init__(self, in_channels, out_channels, n, k=1,
                 stride=1, dilation=1, padding_mode='zeros', deploy=False, use_se=False):
        super().__init__()
