...
class SEBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, rd_ratio: float = 0.0625) -> None:
        ...
        self.reduce = nn.Conv2d(in_channels=in_channels, out_channels=int(in_channels * rd_ratio), kernel_size=1,
                                stride=1, bias=True)
        self.expand = nn.Conv2d(in_channels=int(in_channels * rd_ratio), out_channels=in_channels, kernel_size=1,
                                stride=1, bias=True)
...
class MobileOneBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, inference_mode, kernel_size: int, stride: int = 1,
                 padding: int = 0, dilation: int = 1, groups: int = 1, use_se: bool = False, num_conv_branches: int = 1,
                 ) -> None:
        ...
        rbr_conv = list()
        for _ in range(self.num_conv_branches):#多少个分支结构  3*3的深度可分离卷积结构--
            rbr_conv.append(self._conv_bn(kernel_size=kernel_size, padding=padding))
        self.rbr_conv = nn.ModuleList(rbr_conv)

        # Re-parameterizable scale branch   深度可分离卷积的1*1卷积分支
        self.rbr_scale = None
        if kernel_size > 1:#如果是k = 1的卷积将没有其他1*1的分支
...
class MobileOne(nn.Module):
    ...
    def __init__(self,
        ...
        self.inference_mode = not self.training
    ...
    def _make_stage(self, in_channels, out_channels, num_blocks: int, num_se_blocks: int) -> nn.Sequential:
        ...
        for ix, stride in enumerate(strides):  # 用于训练几个blocks
            ...
            blocks.append(MobileOneBlock(in_channels=in_channels, out_channels=in_channels,
                                         ...
                                         inference_mode=self.inference_mode, use_se=use_se,
